...
```{r}
#incialmente criamos um modelo com as duas variáveis (Age e Infect Risk)
data <- data.frame(LengthStay = LengthStay, CultRatio = CultRatio, DailyCensus = DailyCensus, Facilities = Facilities, NNurses = NNurses, InfectRisk = InfectRisk, Age = Age)
attach(data)
```


```{r}
lm_original <- lm(LengthStay ~  Age + InfectRisk)
anova(lm_original)
```

```{r}
#Serão criados 4 modelos além do inicial, de modo a incluir cada uma das novas variáveis, bem como um teste anova para cada modelo, de  modo a comparar os coeficientes obtidos.
lm_cultratio <- lm(LengthStay ~  Age + InfectRisk + CultRatio, data = data)
anova(lm_cultratio)
```

```{r}
lm_dailycensus <- lm(LengthStay ~  Age + InfectRisk + DailyCensus, data = data)
anova(lm_dailycensus)
```

```{r}
lm_nnurses <- lm(LengthStay ~  Age + InfectRisk + NNurses, data = data)
anova(lm_nnurses)
```

```{r}
lm_facilities <- lm(LengthStay ~  Age + InfectRisk + Facilities, data = data)
anova(lm_facilities)
```

Do teste ANOVA temos que os SSE para cada uma das variáveis incluídas são:
SSEcultratio = 3.247997;
SSEdailycensus = 37.89864;
SSEnnurses = 10.3972;
SSEfacilities = 10.1252;
Dessa maneira, a variável explicativa que constitui a melhor inclusão é a que tem maior SSE, uma vez que isso indica que um maior percentual dos dados é explicado com a inclusão da nova variável. Para os modelos tratados têm-se então que a melhor inclusão é a de DailyCensus.

### Discussão da estatística F parcial para os modelos

Inicialmente podemos notar que os valores F obtidos para as variáveis originais (Age e Infect Risk) sofrem leves alterações para cada um dos 4 modelos, isso se deve ao fato de essa estatística ser uma razão dos valores de MSR para as variáveis originais e MSE para todas as variáveis incluíndo a nova.  Como a variável nova é diferente em cada modelo, o denominador se altera e a estatística F muda.

Em seguida, tem-se os valores de F para cada um dos 4 modelos novos:
Fcultratio = 1.287377;
Fdailycensus = 17.187105;
Fnnurses = 4.231030;
Ffacilities = 4.116160;

Para essa estatística, tem-se que valores elevados levam à rejeição da hipótese nula (coeficiente da variável nula). O valor obtido para a variável DailyCensus indica que sua adição é relevante ao modelo, melhorando o ajuste. Isso é confirmado pelo p-valor encontrado no teste ANOVA: p = 6.722471e-05. Dessa forma, há uma chance muito baixa de que essa estatística tenha sido obtida por acaso, sendo válido aceitar esse risco para o caso dessa variável. 

Por outro lado, a variável CultRatio apresentou o menor valor de F, bem como um valor p maior que 5%. Disso concluímos que a contribuição dessa variável pode ser rejeitada do modelo, uma vez que a hipótese nula foi aceita. 
As outras duas variáveis apresentam estatísticas F intermediárias à essas duas previamente discutidas, e os p-valores obtidos levam à inclusão delas no modelo. Assim conclui-se que estas são relevantes, porém com contribuição menor do que DailyCensus.
...
Construindo uma nova base de dados com os pontos desejados:

```{r}
newdata <- data.frame(LengthStay = LengthStay, DailyCensus = DailyCensus, Age = Age, XrayRatio = XrayRatio)
newdata <- newdata[-c(1:56), ]
attach(newdata)


```

Criação do modelo linear desejado:
```{r}
lm_multicolinear <- lm(LengthStay ~  Age + XrayRatio + DailyCensus, data = newdata)

```

Para a discussão de multicolinearidade do modelo, serão feitas análises de gráficos de dispersão, da matriz de correlação do modelo e VIF do modelo.
```{r}
plot(newdata)
